...
subtest 'global multi_value_array_ref returns arrayrefs' => sub {
    ...
    is_deeply($routers, ['10.0.0.1', '10.0.0.2'], 'routers decoded as two IPs');
...
subtest 'constructor arg overrides global' => sub {
    ...
    $p2->addOptionValue(DHO_ROUTERS(), '10.0.0.1');
...
subtest 'per-object method after construction' => sub {
    ...
    is($r2, '10.0.0.1, 10.0.0.2', 'string value preserved');
};
...
subtest 'csr round-trip with arrayref' => sub {
    ...
    like($routes->[0], qr/10\.0\.0\.0/, 'first route preserved');
};
...
subtest 'inets2 (pairs) with arrayref' => sub {
    ...
    $p->addOptionValue(DHO_STATIC_ROUTES(), '10.0.0.1 192.168.1.1, 10.0.0.2 192.168.1.2');

    my $routes = $p->getOptionValue(DHO_STATIC_ROUTES());
    is(ref $routes, 'ARRAY', 'inets2 is arrayref');
    like($routes->[0], qr/10\.0\.0\.1/, 'first pair preserved');
};
...
subtest 'arrayref mode does not affect input side' => sub {
    ...
    $p->addOptionValue(DHO_ROUTERS(), '10.0.0.1, 10.0.0.2, 10.0.0.3');
